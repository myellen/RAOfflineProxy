...
def import_saved_login(storage: Storage, config_data: dict | None = None) -> dict | None:
    """Seed the login cache from the saved RetroArch token so RetroAchievements
    login works offline immediately.

    Mirrors the documented behaviour ("Start the proxy ... imports your saved
    login"): reads the token from the RetroArch config -- searching every known
    cfg, including muOS's separate retroarch.cheevos.cfg -- and caches a login2
    response under the user. Token-only (no network), so it never delays service
    start; the password->token exchange still happens via the online refresh.
    Does not overwrite a richer login already cached from a real online sign-in.
    """
    config_data = config_data or {}
