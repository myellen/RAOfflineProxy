...
class LinuxInputMapTests(unittest.TestCase):
    ...
    def test_calibrated_code_is_disjoint(self) -> None:
        # Device whose confirm button reports BTN_EAST (normally "back").
        m = input_map.resolve_input_map({"confirm": [KI.BTN_EAST]})
        self.assertIn(KI.BTN_EAST, m["confirm"])
        self.assertNotIn(KI.BTN_EAST, m["back"])  # removed from its old action
        # keyboard fallbacks for confirm are preserved
        self.assertIn(KI.KEY_ENTER, m["confirm"])
        # back keeps its other defaults
    ...
    def test_same_code_under_two_actions_resolves_to_one(self) -> None:
        # A saved file that lists the same code under two actions must not leave
        # the code ambiguous; the later action wins and it's removed from the other.
        m = input_map.resolve_input_map(
    ...
    def test_save_then_load_resolves_via_action_for(self) -> None:
        ...
        # full round-trip: JSON save -> load -> merge -> action_for
        self.assertEqual(input_map.action_for(KI.BTN_EAST, m), "confirm")
    ...
    def test_merge_calibration_preserves_untouched_buttons(self) -> None:
        ...
        self.assertEqual(input_map.action_for(800, m), "confirm")  # re-bound
        self.assertEqual(input_map.action_for(701, m), "back")     # preserved
        self.assertEqual(input_map.action_for(705, m), "right")    # preserved

    def test_merge_calibration_rebinding_onto_another_code_wins(self) -> None:
        # Re-bind confirm onto a code an untouched action (up) already holds.
        saved = {"confirm": [200], "up": [999]}
        merged = input_map.merge_calibration({"confirm": [999]}, saved)
        m = input_map.resolve_input_map(merged)
        self.assertEqual(input_map.action_for(999, m), "confirm")  # new binding wins
    ...
    def test_save_and_load_roundtrip(self) -> None:
        ...
        self.assertEqual(saved, {"confirm": [KI.BTN_EAST]})  # bogus/empty dropped
