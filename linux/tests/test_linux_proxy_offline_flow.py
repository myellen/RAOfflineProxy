"""End-to-end offline-achievement flow against the live proxy server.

Drives the real ProxyRuntimeServer over HTTP (in-process, ephemeral port,
forced offline) the same way RetroArch's RetroAchievements client does:
gameid -> patch -> startsession -> awardachievement. Verifies the proxy serves
cached game data offline, queues the unlock, reflects it in the synthesized
session, and replays it to the server on reconnect (a local mock upstream).
"""
...
class LinuxProxyOfflineFlowTests(unittest.TestCase):
    def test_offline_unlock_is_served_queued_reflected_and_flushed(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                # Seed the cache as if the game was launched online once.
                ...
                # Seed the login the way "Start proxy" does, from the saved token.
                ...
                server.is_online = lambda: False  # deterministic offline, no network
    ...
    def test_run_proxy_service_imports_login_when_online(self) -> None:
        # muOS stores only username+password (no token); starting the proxy while
        # online must exchange them for a token (resolve_credentials) so login
        # keeps working offline. This guards that startup wiring.
        with tempfile.TemporaryDirectory() as tmp:
            cfg = Path(tmp) / "retroarch.cfg"
            cfg.write_text('cheevos_username = "tester"\ncheevos_password = "pw"\n')
            store = storage.Storage(database_path=Path(tmp) / "proxy.sqlite3")
            stop = threading.Event()
            stop.set()  # exit the serve loop immediately
            calls: list = []
            ...
            self.assertEqual(len(calls), 1)  # imported the login once, at startup
    ...
    def _run_offline_phase(self, port, game_hash, gid, user, ach, store) -> None:
        # Offline login must succeed from the seeded token, without any upstream.
        ...
        # The synthesized offline session now includes the just-earned unlock.
